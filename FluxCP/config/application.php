...
return array(
	'ServerAddress'				=> getenv('FLUX_SERVER_ADDRESS') ?: 'localhost',	// This value is the hostname:port under which Flux runs. (e.g., example.com or example.com:80)
	'BaseURI'					=> getenv('FLUX_BASE_URI') !== false ? getenv('FLUX_BASE_URI') : 'fluxcp',	// The base URI is the base web root on which your application lies.
	'ForceHTTPS'				=> getenv('FLUX_FORCE_HTTPS') === '1',	// Enable this when Flux is served through HTTPS.
	'InstallerPassword'			=> getenv('FLUX_INSTALLER_PASSWORD') ?: 'secretpassword',	// Installer/updater password.
	'RequireOwnership'			=> getenv('FLUX_REQUIRE_OWNERSHIP') !== '0',	// Require the executing user to be owner of the FLUX_ROOT/data/ directory tree? (Better for security)
