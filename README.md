...
Il progetto usa `PACKETVER=20141022`, compatibile con la build roBrowser
inclusa nel progetto. Il client roBrowser deve usare lo stesso packetver e
`packetKeys: true`.
...
````
sono le chiavi ufficiali per `PACKETVER=20141022`.
...
Exemplo:  ````./configure --enable-packetver=20230404 && make clean && make server```` ou ````./entrypoint.sh````
## Config RoBrawser
#### Mudar os arquivos
````
...
Il client remoto può leggere direttamente i file dai GRF indicati in
`client/resources/DATA.INI`; non è necessario esportare tutto il contenuto di
`data.grf` nella cartella `client/data`. `CLIENT_AUTOEXTRACT` deve restare
`false`. La cartella `data` viene controllata prima dei GRF e può contenere
solo override o file personalizzati.
...
# Comandos uteis
 ...
 - ./configure --enable-packetver=20230404 && make clean && make server
 - wsproxy -a serve:6900,serve:6121,serve:5121
 - docker-compose -f "docker-compose.yaml" up -d --build
