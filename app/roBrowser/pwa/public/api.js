...
(function ROAPI() {
	...
	ROBrowser.prototype.baseUrl = (function () {
		...
		var scriptUrl = script[script.length - 1].src;
		if (/\/api\.js(?:\?|$)/.test(scriptUrl)) {
	...
	ROBrowser.prototype.start = function Start() {
		switch (this.config.type) {
			...
			case ROBrowser.TYPE.INLINE:
				...
				var projectRoot = /\/applications\/api\/api\.html/.test(this.baseUrl) ? '/@fs/app/' : path.split('/applications/')[0] + '/';
