...
class DB {
	...
	static searchNavigation(query, type) {
		...
		const flattenEntries = value => {
			if (Array.isArray(value)) {
				...
				if (Array.isArray(value[0])) {
					return value.reduce((entries, item) => entries.concat(flattenEntries(item)), []);
				}
			...
			if (value && typeof value === 'object') {
				...
				if (numericKeys.length > 0) {
					return flattenEntries(numericKeys.map(key => value[key]));
				}
				if (
					value.map ||
					value.mapName ||
				...
				return Object.values(value).reduce((entries, item) => entries.concat(flattenEntries(item)), []);
			}
			return [];
		};
		...
		if (type === 'ALL' || type === 'NPC') {
			...
			const npcEntries = flattenEntries(NaviNpcTable);
			for (let i = 0; i < npcEntries.length; i++) {
				const npc = npcEntries[i];
				const mapName = String(npc[0] || npc.mapName || npc.MapName || '').replace(/\.gat$/i, '');
				const npcId = npc[1] ?? npc.id ?? npc.NpcID;
				const npcName = String(npc[4] || npc.name || npc.NpcName || npc.npc_name || '');
				const x = npc[6] ?? npc.x ?? npc.X;
				const y = npc[7] ?? npc.y ?? npc.Y;
				...
				const searchable = `${normalizeSearchText(npcName)} ${normalizeSearchText(mapName)} ${npcId}`;
		...
		if (type === 'ALL' || type === 'MOB') {
			...
			for (let i = 0; i < mobEntries.length; i++) {
				...
				const searchable = `${normalizeSearchText(mobName)} ${normalizeSearchText(mapName)} ${mobId}`;
				if (searchable.includes(query)) {
	...
	/**
	 * Return the monster types configured for a map, independently of the
	 * entities currently synchronized around the player.
	 *
	 * @param {string} mapName
	 * @returns {Array<{id: number, name: string}>}
	 */
	static getMapMonsterTypes(mapName) {
		const wantedMap = String(mapName || '')
			.replace(/\.(rsw|gat)$/i, '')
			.toLowerCase();
		const entries = [];
		const seen = new Set();
		const flatten = value => {
			if (Array.isArray(value)) {
				if (value.length >= 5 && typeof value[0] === 'string') return [value];
				return value.reduce((all, item) => all.concat(flatten(item)), []);
			}
			if (value && typeof value === 'object') return Object.values(value).reduce((all, item) => all.concat(flatten(item)), []);
			return [];
		};

		for (const mob of flatten(NaviMobTable)) {
			const map = String(mob[0] || mob.mapName || '').replace(/\.(rsw|gat)$/i, '').toLowerCase();
			const id = Number(mob[3] || mob.mob_class || mob.id);
			if (map !== wantedMap || !id || seen.has(id)) continue;
			seen.add(id);
			entries.push({ id, name: String(mob[4] || mob.name || this.getMonsterName(id)) });
		}
			...
			try {
				...
				ctx.AddQuestInfo = (
					...
					QuestInfo[QuestID] = {
						...
						Recommended: Recommended === true || Recommended === 1,
				...
				lua.doStringSync(`
					...
					for QuestID, DESC in pairs(QuestInfoList) do
						...
						local questData = {
							...
							Recommended = DESC.Recommended or DESC.recommended or false,
							Pending = DESC.Pending or DESC.pending or false,
						...
						result, msg = AddQuestInfo(QuestID, questData.Title, questData.Summary, questData.IconName, 
												...
												questData.RewardJEXP, questData.Recommended,
												questData.Pending)
						if not result then
							return false, msg
						end
