...
function onOpenRoulette(pkt) {
	...
	if (pkt.result === 0) {
		...
		// The open acknowledgement only contains the current state. The
		// reward slots arrive in the separate roulette-info response.
		const info = new PACKET.CZ.REQ_ROULETTE_INFO();
		Network.sendPacket(info);
	} else {
