...
Navigation.startAutoWalk = function startAutoWalk() {
	if (_autoWalkTimer || !_path.length || !_finalTargetData) {
		return;
	}

	const walk = () => {
		if (!_finalTargetData || !_path.length) {
			this.stopAutoWalk();
			return;
		}
		...
		for (let i = 0; i < _path.length; i++) {
			const distance = Math.abs(_path[i].x - position.x) + Math.abs(_path[i].y - position.y);
		...
		if (closestIndex >= _path.length - 1) {
			this.stopAutoWalk();
			return;
		}

		const targetIndex = Math.min(closestIndex + 8, _path.length - 1);
		const target = _path[targetIndex];
		if (target.isWarp || target.map && target.map !== getCurrentMap()) {
			this.stopAutoWalk();
			return;
		}

		const pkt = PACKET.CZ.REQUEST_MOVE2 ? new PACKET.CZ.REQUEST_MOVE2() : new PACKET.CZ.REQUEST_MOVE();
		pkt.dest[0] = Math.floor(target.x);
		pkt.dest[1] = Math.floor(target.y);
		Network.sendPacket(pkt);
		_autoWalkTimer = setTimeout(walk, 700);
	};
...
Navigation.onSearch = function onSearch() {
	...
	if (results.length === 0) {
		console.warn(`[Navigation] No ${type} navigation result for "${query}".`);
	}
};
...
Navigation.findPath = function findPath(startX, startY, endX, endY) {
	if (_pathFindingWorker && !_pathUpdateLock) {
		...
		const linkEntries = Array.isArray(naviLinkTable)
			? naviLinkTable
			: naviLinkTable && typeof naviLinkTable === 'object'
				? Object.values(naviLinkTable).filter(entry => Array.isArray(entry))
				: [];
		if (linkEntries.length) {
			for (let i = 0; i < linkEntries.length; i++) {
				const warp = linkEntries[i];
				if (!warp || warp.length < 11) {
					continue;
				}

				const srcMap = String(warp[0]).replace(/\.gat$/i, '').toLowerCase();
				const destMap = String(warp[8]).replace(/\.gat$/i, '').toLowerCase();
...
Navigation.setNaviInfo = function setNaviInfo(naviInfo, displayName) {
	...
	if (!parts[0] || !Number.isFinite(x) || !Number.isFinite(y)) {
		return;
	}
