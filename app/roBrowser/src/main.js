...
const startWithConfig = config => {
	...
	Object.keys(config).forEach(key => Configs.set(key, config[key]));
	launch(config);
};

// If ROConfig is already in window (direct access), otherwise wait for the
// parent page to finish constructing it.
if (window.ROConfig) {
	startWithConfig(window.ROConfig);
} else {
	window.addEventListener('robrowser-config-ready', event => startWithConfig(event.detail), { once: true });
