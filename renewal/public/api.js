...
(function ROAPI() {
	...
	ROBrowser.prototype.baseUrl = (function () {
		...
		for (i = 0; i < script.length; i++) {
			if (/\/api\.js(?:\?|$)/.test(script[i].src)) {
				scriptUrl = script[i].src;
				break;
			}
		}
		if (!scriptUrl) {
			scriptUrl = script[script.length - 1].src;
		}
		if (/\/applications\/api\/api\.js(?:\?|$)/.test(scriptUrl)) {
			return location.origin + '/applications/api/api.html';
		}
		if (/\/renewal\/api\.js(?:\?|$)/.test(scriptUrl)) {
			return location.origin + '/renewal/api.html';
		}
		return scriptUrl
	...
	ROBrowser.prototype.start = function Start() {
		switch (this.config.type) {
			...
			case ROBrowser.TYPE.INLINE:
				...
				var projectRoot;
				if (/\/applications\/api\/api\.html/.test(this.baseUrl)) {
					projectRoot = '/@fs/app/';
				} else if (path.indexOf('/renewal/') === 0) {
					projectRoot = '/renewal/';
				} else {
					projectRoot = path.split('/applications/')[0] + '/';
				}

				if (!document.querySelector('script[type="importmap"]')) {
					var sharedScript = document.createElement('script');
					sharedScript.src = projectRoot + 'shared/importmap.js';
