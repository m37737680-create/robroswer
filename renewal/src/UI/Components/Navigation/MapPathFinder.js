...
MapPathFinder.findPathBetweenMaps = function findPathBetweenMaps(
	...
	startMap = normalizeMap(startMap);
	endMap = normalizeMap(endMap);
	...
	const links = asList(naviLinkTable);
	const navigationDistances = asList(naviLinkDistanceTable);
	if (!links.length) {
		return null;
	}
	...
	for (let i = 0; i < links.length; i++) {
		...
		const srcMap = normalizeMap(warp[0]);
		const warpId = warp[1];
		const warpType = Number(warp[2]);
		const destMap = normalizeMap(warp[8]);

		// Skip invalid warps or warps of types not in warpTypes
		if (!srcMap || !destMap || warpTypes.map(Number).indexOf(warpType) === -1) {
			continue;
		}
	...
	if (navigationDistances.length) {
		for (let i = 0; i < navigationDistances.length; i++) {
			if (typeof navigationDistances[i] === 'string') {
				const mapName = normalizeMap(navigationDistances[i]);
				if (i + 2 < navigationDistances.length) {
					//var numLinks = naviLinkDistanceTable[i + 1]; // UNUSED
					const linksData = navigationDistances[i + 2];
					...
					if (Array.isArray(linksData)) {
						...
						for (let j = 0; j < linksData.length; j++) {
							...
							const warpKey = normalizeMap(mapName) + '_' + linkId;
							...
							if (warpTypes.map(Number).indexOf(Number(warpInfo.type)) === -1) {
								continue;
							}
							...
							for (let k = 1; k < linksData[j].length; k++) {
								...
								if (Array.isArray(destData) && destData.length >= 3) {
									const destMapName = normalizeMap(destData[0]);
